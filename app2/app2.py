...
@app2.route("/info", methods = ['GET'])
def get_all_contact():
    data = Suhu.find({})
    return 'INI WEB SERVER APP 2 \n ' + dumps(data)

@app2.route('/input/suhu', methods = ["POST"])
def post():
    sekarang = dtm.now()
    ...
    return str(data) + "\n berhasil input data ke Database ... BTW, INI WEB SERVER APP 2"

@app2.route('/')
def hello_world():
    return 'Assalamualaikum, ini web server App2 :)'
